...
public class ClassUtils {
	...
	private static List getAllInterfacesExclude(Class clazz, String itfNames[],
			List list) {
		if (list == null) {
			list = new ArrayList();
		}
		...
		try {
			...
			for (int i = 0; i < constructors.length; i++) {
				...
				if (isAssignable(expectedTypes, creator.getParameterTypes())) {
					if (constructor == null) {
						constructor = creator;
					} else if (isAssignable(creator.getParameterTypes(),
			...
			String msg = (new StringBuilder())
					...
					.append(clz.getName())
					.append(", ")
					.append(clz.getProtectionDomain().getCodeSource()
							.getLocation()).append(", ")
	...
	public static Method getMethod(Class clz, String methodName,
		...
		try {
			...
			for (int i = 0; i < methods.length; i++) {
				...
				if (isAssignable(_method.getParameterTypes(),
	...
	public static Class loadClass(final String name)
		...
		if (obj instanceof Class)
			...
			throw new ClassNotFoundException(
					"Not Found the class which name of" + name);
	}
